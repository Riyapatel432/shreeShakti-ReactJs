...
const trimmed =
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEdit) {
        res = await dispatch(updateUnit({ id: editingId, data: form }));
      } else {
        res = await dispatch(addUnit(form));
      }
...
const handleDeleteConfirm = async () => {
  const res = await dispatch(deleteUnit(deleteId));

  if (res.meta?.requestStatus === "fulfilled") {
    setSuccessMessage("Unit deleted successfully");
  } else {
    setServerError(
      res.payload?.message || "Failed to delete unit"
    );
  }
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Breadcrumbs separator={<NavIcon fontSize="small" />}>
          ...
          <Typography color="warning.main">Unit </Typography>
        </Breadcrumbs>
      </Box>
